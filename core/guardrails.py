...
SYNTAX_LOOKUP = {
    ...
    "name 'get_shape_blueprint' is not defined": "CRITICAL: get_shape_blueprint() is an internal tool, NOT a build123d function. Do NOT write get_shape_blueprint() in your Python code. Use it as a thinking tool only, then write standard build123d code.",
    "name 'Direction' is not defined": "'Direction' is not a build123d object. To set direction/orientation, use the Axis class (e.g., Axis.Z) or simply use 'with Locations((X, Y, Z)):' to position parts.",
    "name 'difference' is not defined": "'difference()' is OpenSCAD syntax. In build123d, use the minus operator: `final_part = outer_shape - inner_shape`. Do NOT use difference().",
    "has no attribute 'finalize'": "Do NOT call .finalize() on any object. In build123d, simply assign 'final_part = p.part' after your BuildPart context block.",
    "A face or sketch must be provided": "You called extrude() without first creating a 2D sketch. Instead of using extrude(), use 3D primitives directly: Box(length, width, height) or Cylinder(radius=..., height=...) inside a BuildPart context.",
    "has no attribute 'solve'": "Do NOT call .solve() on any object. Simply build your geometry using primitives and assign it to 'final_part'.",
    "No objects to create a hull": "hull() requires at least 2 objects to wrap. Avoid using hull() - instead build the shape directly using Box() and Cylinder() primitives.",
    "has no attribute 'make_solid'": "There is no '.make_solid()' method in build123d. Geometry is already solid when created with Box(), Cylinder() etc. inside a BuildPart context.",
    "name 'make_solid' is not defined": "There is no 'make_solid()' function in build123d. Geometry is automatically solid. Remove make_solid() and just use your primitives directly.",
    "'NoneType' object has no attribute 'to_python'": "You returned None from a geometry call. Make sure your final geometry is assigned to 'final_part = p.part' and that the BuildPart context actually created objects.",
    "FilletPolyline applies to": "FilletPolyline is a 2D operation for BuildLine, not BuildPart. Do NOT use FilletPolyline in a BuildPart context. Keep geometry simple using only Box() and Cylinder() primitives.",
    "invalid syntax": "Your script has a Python syntax error. Check for missing colons after 'with' statements, mismatched parentheses, or incorrect indentation. The script must be valid Python.",
