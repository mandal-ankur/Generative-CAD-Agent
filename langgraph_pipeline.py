...
def planner_node(state: CADState) -> dict:
    ...
    rb = ""
    if "sphere" in p_lower:
        rb = "sphere"
    elif "stand" in p_lower or "cantilever" in p_lower:
        rb = "cantilever"
    elif "bracket" in p_lower:
        rb = "bracket"
    elif design_intent.get("is_container"):
        rb = "hollow_container"
    elif "overhang" in p_lower or "floating" in p_lower or "balcony" in p_lower:
        rb = "overhang_support"
    elif "cone" in p_lower:
        rb = "cone"
    elif "cube" in p_lower or "box" in p_lower or "plate" in p_lower or "block" in p_lower:
        rb = "cube"
    elif "cylinder" in p_lower or "disk" in p_lower:
        rb = "cylinder"
    elif "gear" in p_lower:
        rb = "gear"
    elif "pyramid" in p_lower:
        rb = "pyramid"
    ...
    out_dir = os.path.join(CAD_DIR, _slugify(prompt))
...
def executor_node(state: CADState) -> dict:
    ...
    if not code_block:
        ...
        return {"code_block": None, "final_part": None, "exec_error": err, "error_history": error_history, "syntax_errors": syntax_errors}

    code_block = code_block.replace("```python", "").replace("```", "").strip()
    safe = out_dir.replace("\\", "/")
    code_block = re.sub(r"export_stl\s*\([^)]+\)", f"export_stl(final_part, '{safe}/cad.stl')", code_block)
    code_block = re.sub(r"export_step\s*\([^)]+\)", f"export_step(final_part, '{safe}/cad.step')", code_block)
    ...
    try:
        ...
        try:
            ...
            pass
        return {"code_block": code_block, "final_part": final_part, "exec_error": None, "error_history": error_history, "syntax_errors": syntax_errors}
        ...
        return {"code_block": code_block, "final_part": None, "exec_error": smart_err, "error_history": error_history, "syntax_errors": syntax_errors}
...
def validator_node(state: CADState) -> dict:
    ...
    if final_part is None:
        return {"is_valid": False, "validation_report": {"message": state.get("exec_error", "Execution failed")}, "error_history": error_history, "physics_errors": physics_errors}

    is_valid, report = validate_geometry(final_part, intent=state.get("design_intent", {}), user_prompt=state["user_prompt"], z_offset=0.0)
    if not is_valid:
        physics_errors += 1
    ...
    return {"is_valid": is_valid, "validation_report": report, "error_history": error_history, "physics_errors": physics_errors}
...
def exporter_node(state: CADState) -> dict:
    ...
    return {"success": bool(step and stl), "step_path": os.path.join(state["output_dir"], step) if step else None, "stl_path": os.path.join(state["output_dir"], stl) if stl else None, "final_error": None}
...
def build_graph():
    ...
    g.add_conditional_edges("validator", route_after_validation, {"planner": "planner", "exporter": "exporter", END: END})
    return g.compile()
...
def generate_cad_part_graph(user_prompt: str, prompt_id: str = "default") -> dict:
    ...
    initial: CADState = {
        ...
        "design_intent": {}, "rag_context": [], "requires_blueprint": "", "llm_prompt": "", "output_dir": "",
        ...
        "is_valid": False, "validation_report": {}, "step_path": None, "stl_path": None, "success": False, "final_error": None,
    }
...
if __name__ == "__main__":
    result = generate_cad_part_graph("Make a hollow cylindrical cup with 15mm outer radius and 2mm walls", prompt_id="test_001")
